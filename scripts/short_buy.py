...
for _, row in bullish_df.iterrows():
    ...
    # Get gape value from rsi_diver.csv
    gape_value = row.get('gape', 0)  # Default to 0 if gape column not found
...
# =========================
# Combine All Signals (Only Bullish)
# =========================
all_signals = bullish_signals

if all_signals:
    df_signals = pd.DataFrame(all_signals)

    # Sort by symbol
    df_signals = df_signals.sort_values(['symbol']).reset_index(drop=True)
