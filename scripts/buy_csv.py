...
def create_uptrend_downtrend_signals():
    ...
    downtrend_file = os.path.join(output_dir, 'downtrand.csv')  # ← এখন ./csv/ তে থাকবে
    ...
    if uptrend_signals:
        ...
        print(f"✅ Uptrend updated: {len(final_up_df)} symbols")
    ...
    if downtrend_signals:
        ...
        print(f"✅ Downtrend updated: {len(final_down_df)} symbols")
...
if __name__ == "__main__":
    ...
    # Signal detection
    # --------------------------------------------------
    for symbol, info in latest_data.items():  # <-- এই লাইনটি ইন্ডেন্ট করুন
        symbol_dir = os.path.join(trand_base_dir, symbol)
        high_file = os.path.join(symbol_dir, 'high.csv')
        low_file = os.path.join(symbol_dir, 'low.csv')
...
if __name__ == "__main__":  # <-- এখানে __name__ ঠিক করুন
    main()
